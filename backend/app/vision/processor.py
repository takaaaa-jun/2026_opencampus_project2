...
class MediaPipeProcessor:
    def __init__(
        ...
        missing = [path for path in (hand_model_path, pose_model_path) if not path.is_file()]
        if missing:
        ...
        self._actions = ActionEngine()
        self._actions.update_config({'clapThreshold': clap_threshold})
    ...
    def process(self, frame_rgb: np.ndarray, received_at_ms: int) -> dict[str, Any]:
        with self._lock:
            if self._closed:
                raise RuntimeError('MediaPipeProcessor is closed.')
            return self._process_locked(frame_rgb, received_at_ms)

    def _process_locked(self, frame_rgb: np.ndarray, received_at_ms: int) -> dict[str, Any]:
        started = time.perf_counter()
        height, width = frame_rgb.shape[:2]

        timestamp_ms = max(int(time.monotonic_ns() // 1_000_000), self._last_timestamp_ms + 1)
    ...
    def _serialize_hands(result: Any) -> list[dict[str, Any]]:
        ...
        for index, landmarks in enumerate(result.hand_landmarks or []):
            ...
            if index < len(handedness_list) and handedness_list[index]:
                ...
                handedness = str(getattr(category, 'category_name', 'unknown')).lower()
            hands.append(
                {
